    ...
    if (!this.interval) {
      this.interval = setTimeout(() => {
        ...
        if (div >= 1 - PERCENT) {
          ...
          if (parseInt(rest) !== 0) {
            if (rest <= added) {
              this.ref.current.scrollTo({ y: this.state.contentSize.height });
              this.setState({ addedHeight: this.state.addedHeight + rest }, cb);
            } else {
              this.ref.current.scrollTo({ y: this.state.scrollY + added });
              this.setState(
          ...
          if (parseInt(rest) !== 0) {
            if (rest <= added) {
              this.ref.current.scrollTo({ y: 0 });
              this.setState({ addedHeight: 0 }, cb);
            } else {
              this.ref.current.scrollTo({ y: this.state.scrollY - added });
              this.setState(
                { addedHeight: this.state.addedHeight - added },
